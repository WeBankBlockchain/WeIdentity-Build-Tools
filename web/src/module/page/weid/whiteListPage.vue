<template>
  <section class="content app_main">
    ...
    <div class='app_view'>
      <el-table :data="page.whiteList.slice((page.pageIndex-1)*page.pageSize, page.pageIndex*page.pageSize)" border="true" align="center">
          <el-table-column prop="type" label="白名单名称" align="center">
            <template slot-scope='scope'>
              <span class='long_words' :title='scope.row.type'>{{scope.row.type}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="createTime" label="创建时间" align="center" ></el-table-column>
          <el-table-column label="操作" align="center" width='350'>
            <template slot-scope='scope'>
              <el-button  type="primary" class="btn" @click="showAllIssuer(scope.row.type)" style="width:100px;">查看白名单</el-button>
              <el-button  type="primary" class="btn" @click="showAddIssuer(scope.row.type)" style="width:180px;">添加WeID到这个白名单</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false">
      <el-table :data="dialog.whiteWeIdListPage.whiteWeIdList.slice((dialog.whiteWeIdListPage.pageIndex-1)*dialog.whiteWeIdListPage.pageSize, dialog.whiteWeIdListPage.pageIndex*dialog.whiteWeIdListPage.pageSize)" border="true" cellpadding="0" cellspacing="0">
...
export default {
  ...
  methods: {
    ...
    getAllIssuerInType (issuerType) {
      ...
      API.doPost('getAllIssuerInType', formData).then(res => {
        if (res.data.errorCode === 0) {
    ...
    init () {
      API.doGet('getIssuerTypeList').then(res => {
        if (res.data.errorCode === 0) {
          this.page.whiteList = res.data.result
