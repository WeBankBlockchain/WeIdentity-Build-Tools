...
(function ($) {
  ...
  $.fn.extend({
	 renderData: function(template, obj) {
		 $(this).html("");
		 if (obj instanceof Array) {
			 for(var i = 0; i < obj.length; i++) {
				obj[i]["index"] = i + 1;
				$(this).append($(dataRender(template, obj[i])).removeClass("display-none"));
			}
		 } else {
			$(this).html($(dataRender(template, obj)));
		}
	 },
	 loadSelect: function(url,value,text,caller) {
		 var $this = this;
  ...
  function dataRender(templet,data) {
    var s = templet.match(/\$\{(\S*)\}/g);
    for(var i = 0; i < s.length; i++) {
        var key = s[i].match(/{(\S*)}/)[1];
        key = key.replace(/^\s+/, '').replace(/\s+$/, '');
        templet = templet.replace(s[i],data[key]);
    }
    return templet;
  }
  var isLisent = false;
  $.confirm = function(message, caller) {
  	$("#confirmBody").html("<p style='white-space:normal'>"+message+"</p>");
...
function goTo(thisObj, url) {
	var disabled = $(thisObj).attr("class").indexOf("disabled");
    if(disabled > 0) return;
    document.location.href=url;
}

function showWeId(weId) {
	$("#messageBody").html("<p>"+weId+"</p>");
	$("#modal-message").modal();
}
...
function showMessage(thisObj, message, event) {
    ...
    var showbox=$('<div>'+message+'</div>').css({
                  padding:'2px 5px',
                  position:'fixed',
                  zIndex: 9999,
                  fontSize: "12px",
